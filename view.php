	...
	echo '</div>';
        ...
        if($sql_result->numRows() > 0) {
            while($row = $sql_result->fetchRow()) {
                echo '<div class="brick wow zoomIn" data-wow-duration="1.5s"><a href="' .WB_URL.$file_dir.$row["bildname"].'" class="big'.
                     ( $usebrax ? '' : 'lb_image_link' ).
                     '" title="'.$row["bildbeschreibung"].'"'.
                     ( $usebrax ? '' : ' rel="lightbox[' . POST_ID . ']"' ).
                     '>'.
                     '<img class="' . ( $usebrax ? 'thumbnail' : 'lb_image' ).
                     '" src="'.WB_URL.$file_dir.$row["bildname"].'" alt="'.$post['title'].'" title="'.$row["bildbeschreibung"].'" /></a></div>'.
                     ( $usebrax ? '' : '' );
            }
        }
