		...
		if($settings['use_captcha'])
        {
			if(isset($_POST['captcha']) AND $_POST['captcha'] != '')
            {
				...
				if(!isset($_POST['captcha']) OR !isset($_SESSION['captcha']) OR $_POST['captcha'] != $_SESSION['captcha'])
                {
					...
					header("Location: ".WB_URL."/modules/news_img/comment.php?post_id=".$post_id."&section_id=".$section_id."" );
	                exit( 0 );
				}
			}
            else
...
else
{
	if( isset($_GET['post_id']) AND is_numeric($_GET['post_id'])
        AND isset($_GET['section_id']) AND is_numeric($_GET['section_id']) )
    {
 		header("Location: ".WB_URL."/modules/news_img/comment.php?post_id=".($_GET['post_id'])."&section_id=".($_GET['section_id'])."" ) ;
	    exit( 0 );
    }
	else
    {
