...
<?php
    ...
    <input type="hidden" name="savegoback" id="savegoback" value="" />
        ...
        while ($post = $query_posts->fetchRow()) {
            ...
            <td <?php if ($setting_view_order == 0) {
                echo 'class="dragdrop_item"';
            } ?>>&nbsp;</td>
                <td>
            ...
            if ($query_title->numRows() > 0) {
                ...
                <td>
                <a href="<?php echo WB_URL; ?>/modules/news_img/activate_post.php?page_id=<?php echo $page_id; ?>&amp;section_id=<?php echo $section_id; ?>&amp;post_id=<?php echo $post_id_key; ?>&amp;value=<?php echo $post['active']!=0 ? '0':'1'; ?>');" title="<?php if ($post['active'] == 1) {
                echo $MOD_NEWS_IMG['DEACTIVATE_POST'];
            } else {
                echo $MOD_NEWS_IMG['ACTIVATE_POST'];
            } ?>">
                    <?php if ($post['active'] == 1) {
                echo '<span class="fa fa-fw fa-eye nwi-active"></span>';
            } else {
                echo '<span class="fa fa-fw fa-eye-slash nwi-inactive"></span>';
            } ?></a>
                </td>
                <td>
<?php
...
<form method="post" action="<?php echo WB_URL; ?>/modules/news_img/save_tag.php">
    ...
    <input type="submit" class="mod_img_news_add" value="<?php echo $MOD_NEWS_IMG['ADD_TAG']; ?>" /><br />
</form>
...
<?php
        foreach($tags as $tag_id => $tag) {
            $tag_id_key = $admin->getIDKEY($tag_id);
?>
            <tr>
                <td style="width:20px">
